...
## このシステムによって実現できること
Zoom上でこのシステムを利用することによってZoomFatigueの要因となるような状況を避けることができます。
- アバタを通してZoom上で自身を表現できるため、他者からの直接的な視線を防ぐことが可能です。
- Zoom上で伝わりづらい非言語情報をアバタが強調して表現してくれます。また、閾値の変更や協調の有無などを決めることができます。
- 本システムはVRM形式のアバタに対応しているため、自分の好きなアバタをこのシステムに組み込むことができます。
- Zoom上ではカメラから外れて離席がしずらいですが、本システムでは離席すると一時的に自動でアバタが自然に振る舞ってくれます。
- 自身が横を向いていても、アバタは基本的に正面を向くように設定されているため、真正面を向き続ける必要性を無くしてくれます。
